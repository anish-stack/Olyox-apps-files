...
export const fetchPastRidesData = async () => {
  try {
    ...
    const response = await axios.get("https://www.appv2.olyox.com/api/v1/user/my-past-rides", {
      headers: { Authorization: `Bearer ${token}` },
    })
    // console.log("Past rides response data s:", response.data)
    return response.data?.data || []
  } catch (error) {
    console.error("Error fetching past rides:", error.response?.data?.message || error.message)
...
export const fetchDirectionsPolyline = async (pickup, dropoff) => {
  try {
    const response = await axios.post("https://www.appv2.olyox.com/directions", {
      pickup: {
